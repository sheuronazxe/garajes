...
def generar(datos, fecha):
    ...
    i = 0
    ancla = 795
    while datos.item(i,0):
        nombre = datos.item(i,0).text()
        garaje = datos.item(i,1).text() if datos.item(i,1) else ''
        alquiler = datos.item(i,2).text() if datos.item(i,2) else ''
        i += 1
        ...
        pdf.drawCentredString(530, ancla-34, alquiler + " €")
